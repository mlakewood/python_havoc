...
#    @settings(max_examples=1000)
    ...
    def test_convert_to_tc_fail_example(self):
        command = {'loss_random': {'percent': 85}, 'delay': {'jitter': 3281, 'distribution': None, 'correlation': 39, 'time': 2637}, 'limit': {'packets': 77189}, 'corrupt': None, 'duplicate': None, 'rate': None}

        cmd = ContainerSystem.build_tc_command(command)

        expected_output = """tc qdisc add dev eth0 root handle 1: htb default 1;
tc class add dev eth0 parent 1: classid 1:1 htb rate 1000mbit;
tc qdisc add dev eth0 parent 1:1 handle 2: netem limit 77189 delay 2637ms 3281ms 39% loss random 85%;"""
...
class TestMockGenerateSystemState(BaseTest):
    ...
    @given(st.data())
    def test_next_state_ingress_fail(self, data):
        with patch.multiple(ContainerSystem,
                            stop_container=DEFAULT,
                            start_container=DEFAULT,
                            fix_network=DEFAULT,
                            fault_egress=DEFAULT,
                            fail_ingress=DEFAULT,
